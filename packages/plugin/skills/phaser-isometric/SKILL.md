...
description: "Use this skill when building an isometric or 2.5D diamond-grid game in Phaser 4 with the phaser-isometric package, or any other axonometry through its matrix spec — any elevation and any orientation, not just the usual 45°. Covers installing the Scene plugin, the projection and its centre-of-top-face convention, IsoSprite, tie-free depth ordering with bands, elevation-aware cell picking, view culling, isometric camera bounds, cell hit areas (and why a rhombus one is the wrong shape off 45°) and the debug overlay. Triggers on: isometric, iso grid, diamond tiles, axonometric, dimetric, oblique, orientation, elevation angle, IsoSprite, isoScenePlugin, depth sorting, elevation, tile picking, 2.5D, phaser-isometric."
---
...
The `matrix` form is not an escape hatch — it is a **general axonometry**, and it is exact. Any elevation and any orientation can be written down, which is what a vector editor's isometric-grid panel produces when it is set to something other than the usual 45°. For elevation `e`, orientation `o` and cube scale `S`:
...
Two things follow, and the second one bites. **The one thing that cannot be expressed is roll**: elevation only ever moves a sprite along screen `-y` (there is no `z` term in `screenX`), so a view rotated in the picture plane is out of reach. And **when the orientation is not 45°, a cell stops being a rhombus** and becomes a skewed parallelogram: `a !== -c`, or `b !== d`, or both. Use `isRhombus(projection)` to ask. Everything in the package is generic over the matrix — `project`, `unproject`, `cornersOf`, `pick`, `worldBounds`, `cullBounds`, the debug overlay — with one exception that has its own guard: a hit area, see "Cell hit areas".
...
```ts
this.iso.makeCellHitArea(tile);                                 // the cell's real shape — use this one
this.iso.makeDiamondHitArea(tile, { tileWidth: 96, tileHeight: 48 });   // an explicit rhombus of your own
```
...
The package ships a command. Use it to **check an assumption instead of guessing** — before writing the Scene, or when a projection is behaving in a way you cannot explain:

```sh
npx phaser-isometric diagnose --tile 96x48 --grid 24x24
```

It prints one card: the matrix and its determinant, how far the matrix is from singular, the depth layout with the exact row at which keys stop being distinguishable, the world bounds for that grid, and the round-trip error. One call instead of five, and no browser.

The command is **inside this package**, so it is by construction the version you installed — there is no second artifact that can drift from it. `--json` gives the same facts as data. Add `--strict` to make warnings exit non-zero for CI; without it a warning is printed and the exit code stays 0.

What it reports as a **warning** is a real configuration that will surprise you later: tile centres landing on half pixels, cells that are skewed parallelograms rather than rhombuses (with the factor by which a diamond hit area would over-cover them), a grid axis running backwards for the default depth key, an `elevationStep` of 0 (which a raw `matrix` spec defaults to, so elevation silently does nothing), a near-singular matrix, a grid reaching past the row the depth layout was validated for. What it reports as an **error** is a configuration the constructors reject outright, and it names the correction:

```sh
$ npx phaser-isometric diagnose --matrix 1,1,1,1
## ERROR
kind=IsoConfigError
symptom=the projection matrix is not invertible (det = 0, a=1 b=1 c=1 d=1)
fix=columns (a,b) and (c,d) are collinear: change one of them, or use the diamond preset
```

Run `npx phaser-isometric help` for every flag.
...
| `makeCellHitArea` | `(target) => target` | The cell's real shape under this projection; correct on any axonometry |
| `makeDiamondHitArea` | `(target, { tileWidth?, tileHeight? }?) => target` | A rhombus; defaults to the projection's tile size, and throws rather than default it from a non-rhombus projection |
...
4. **The performance promise, with the clause that belongs to it.** Driving 500 simultaneously active `IsoSprite`s — each re-projected and re-depth-sorted every single frame across a 100x100-cell world, with a following camera and view culling keeping 23-59 of them on screen at any instant — phaser-isometric held the browser's own frame cadence at the median: an 8.3 ms median frame period, at least 2x inside the 16.6 ms / 60 fps budget (a handful of single-frame outliers up to 34 ms also appeared — this is a median claim, not a claim that every frame met budget). That median reproduced identically in all eighteen runs of two separate campaigns, two days apart, on an otherwise idle machine. Measured directly — bracketing the plugin's own main-thread update-and-render-submission work, which excludes GPU rasterisation happening asynchronously outside that bracket — per-frame CPU-side cost averaged 0.17-0.25 ms at 500 active entities against a ~0.05-0.07 ms baseline for the same page with none: roughly 2.5-5x the baseline (a range, not a constant, since both terms sit near the browser's ~100 us timing floor, and the second campaign extended both ends downward without moving the ratio). Conditions, which travel with the number: Apple M1 Max laptop, not CI; Chromium headless, WebGL through SwiftShader software rendering. **"500 active" is not "500 drawn on screen"** — culling kept 23-59 of them visible, and the drawn-count claim was never measured.
