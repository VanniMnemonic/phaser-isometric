...
What it reports as a **warning** is a real configuration that will surprise you later: tile centres landing on half pixels, an `elevationStep` of 0 (which a raw `matrix` spec defaults to, so elevation silently does nothing), a near-singular matrix, a grid reaching past the row the depth layout was validated for. What it reports as an **error** is a configuration the constructors reject outright, and it names the correction:
