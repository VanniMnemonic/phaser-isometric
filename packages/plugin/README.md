...
frame cadence at the median across nine independent runs: an 8.3 ms median frame period,
at least 2x inside the 16.6 ms / 60 fps budget (a handful of single-frame outliers up to
34 ms also appeared — this is a median claim, not a claim that every frame met budget).
...
per-frame CPU-side cost averaged 0.19-0.25 ms at 500 active entities against a
~0.06-0.07 ms baseline for the same page with none: roughly 2.5-5x the baseline (a range,
not a constant, since both terms sit near the browser's ~100 us timing floor).
